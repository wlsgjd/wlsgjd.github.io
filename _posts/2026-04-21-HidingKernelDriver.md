...
## PsLoadedModuleList
일반적으로 윈도우에서 드라이버가 로드되면 PsLoadedModuleList에 기록됩니다.<br>
이는 현재 로드된 드라이버 목록입니다. windbg에서 lmk 명령어를 사용해 확인한 리스트와 동일합니다.

대부분 해킹툴들은 **iqvw64e.sys**와 같은 취약 드라이버를 로드하고, 사용이 완료되면 언로드하기 때문에 PsLoadedModuleList에는 흔적이 남지 않습니다.
```
2: kd> lmk
...
## MmUnloadedDrivers
언로드 된 드라이버 목록입니다. 마찬가지로 windbg에서 lmk 명령어를 통해 확인할 수 있습니다.
```
Unloaded modules:
...
다음과 같은 스크립트[MmUnloadedDrivers](https://github.com/wlsgjd/Windbg-Scripts/blob/main/MmUnloadedDrivers)를 통해서도 확인할 수 있습니다.
```
...
### _MM_UNLOADED_DRIVER
MmUnloadedDrivers은 **_MM_UNLOADED_DRIVER**에 대한 배열 메모리입니다.
```cpp
...
windbg를 통해 확인해보면 다음과 같이 구성되어 있습니다.
```
...
배열 메모리이기 때문에 내용을 쉽게 확인할 수 있습니다.
```
...
드라이버를 언로드하게 되면 OS 내부에서 MiRememberUnloadedDriver이 호출됩니다.<br>
해당 함수는 호출 시 <code>LDR_DATA_TABLE_ENTRY</code>를 전달 받으며 MmUnloadedDrivers에 내용을 기록합니다.<br>
```
...
함수 내부를 자세히 보면 전달 받은 **BaseDllName**의 길이를 검사합니다. 길이가 0인 경우 기록하지 않습니다.
```
...
[kdmapper](https://github.com/TheCruZ/kdmapper)는 이를 이용하여 MmUnloadedDrivers에 기록되지 않도록 길이와 버퍼를 제거합니다.
```cpp
...
## PiDDBCache
PiDDBCache은 드라이버가 로드되면서 남는 캐시 데이터입니다.<br>다음과 같은 스크립트 [piddbcachelist](https://github.com/wlsgjd/Windbg-Scripts/blob/main/piddbcachelist)를 통해서도 간단하게 확인이 가능합니다.
```
...
PiDDBCache은 <code>_PI_DDB_CACHE_ENTRY</code>를 기록하며 구조는 다음과 같습니다.
```cpp
...
PiDDBCacheTable, PiDDBCacheList에 기록되며 각각 AVL 트리, 리스트 구조를 가집니다.<br>아래는 windbg에서 확인한 리스트 내용 중 일부입니다.
```
...
kdmapper는 각 요소에 대한 데이터를 모두 제거합니다.
```cpp
...
KernelHashBucketList은 드라이버가 로드될 때 기록되는 해시 리스트입니다.<br>
ci.dll에 존재하는 전역 변수이며 일반적인 방식으로는 가져오는게 불가능합니다.

KDMapper에서도 다음과 같이 시그니처를 통해 찾는 것으로 보입니다.
```cpp
...
해당 시그니처를 검색하면 다음과 같습니다. <code>CI!g_KernelHashBucketList</code>를 찾는 것으로 보입니다.
```
...
g_KernelHashBucketList에는 <code>_HashBucketEntry</code>이 리스트 구조로 기록됩니다.
```cpp
...
windbg를 통해 확인해보면 다음과 같습니다. 드라이버 경로와 인증서 관련 해시가 기록되는 것으로 보입니다.
```
...
kdmapper에서는 리스트 내에서 대상과 일치하는 드라이버를 찾고 링크를 끊어 버립니다.
```cpp
