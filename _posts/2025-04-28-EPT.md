...
CPU가 게스트에 진입하는 경우를 VM-Entry(또는 VM-Enter라고도 표현함), 게스트에서 호스트로 빠져나오는 경우를 VM-Exit라고 표현합니다.
...
VMXON을 실행하기 위해서는 먼저 VMX 기능을 활성화해야 합니다. CR4.VMXE 비트를 1로 변경하는 경우 활성화됩니다.
```
...
### VMXON
VMXON을 실행하기 위해서는 VMXON Region을 먼저 할당합니다.
