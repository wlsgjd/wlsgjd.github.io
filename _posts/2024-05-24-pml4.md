...
정리하자면, 물리 주소 변환에 사용되는 가상 주소는 다음과 같은 구조입니다.
...
추가로, 최신 프로세서는 한단계 확장된 5-level panging 기술을 지원합니다. 기존에 사용되던 4-level paging에서 PML4와 유사한 PML5가 추가되었습니다. 
...
현재 테스트 환경에서는 PML5가 활성화되어있지 않기 때문에, 추후에 기회가 된다면 해당 부분도 리뷰해보겠습니다.
